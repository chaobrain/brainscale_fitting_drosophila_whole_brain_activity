...
Input = \\sum_i firing_rate_token_i @ weight_i
...
class BatchedMatMulOp(brainscale.ETraceOp):
    def xw_to_y(self, inputs, weights):
        return jax.vmap(lambda x, w: x @ w['weight'] + w['bias'])(inputs, weights).sum(axis=0)


class BatchedLinear(brainstate.nn.Module):
    def __init__(
        self,
        n_in: int,
        n_out: int,
        ...
        # weights
        weight = brainstate.augment.vmap(lambda: brainstate.init.param(w_init, [n_in, n_out]), axis_size=n_batch)()
        bias = brainstate.augment.vmap(lambda: brainstate.init.param(b_init, [n_out]), axis_size=n_batch)()
        params = dict(weight=weight, bias=bias)
        # weight + op
        self.weight_op = param_type(params, op=BatchedMatMulOp())
...
class NeuralActivity(brainstate.nn.Module):
    ...
    def __init__(
        ...
        self.bin_bias = self.bin_size.to_decimal(u.Hz) / 2
        ...
        self.neuropil2neuron = BatchedLinear(
            self.n_neuropil,
            self.pop.varshape[0],
            self.n_hist,
    ...
    def neuropil_fr_to_embedding(self, neuropil_fr: u.Quantity[u.Hz]):
        ...
        def convert(fr):
            ...
            embed = bin_indices + self.bin_bias
            return embed

        fun = convert
        for _ in range(neuropil_fr.ndim - 1):
            fun = jax.vmap(fun)
        return fun(neuropil_fr)
    ...
    def iter_data(
        ...
        for i in range(1 + self.n_hist, self.n_time, batch_size):
            ...
            yield (
                input_neuropil_fr,  # [n_batch, n_hist, n_neuropil]
                output_neuropil_fr,  # [n_batch, n_neuropil]
            )
...
def parse_setting(fn: str):
    ...
    max_firing_rate = float(settings[2]) * u.Hz
    etrace_decay = float(settings[3])
    loss_fn = settings[4]
    conn_param_type = getattr(brainscale, settings[5])
    input_param_type = getattr(brainscale, settings[6])
    scale_factor = float(settings[7]) * u.mV
    n_rank = int(settings[8])
    sim_before_train = float(settings[9])
    seed = int(settings[10])
...
def first_round_train():
    ...
    if checkpoint_path is None:
        ...
        lr, etrace_decay, scale_factor, bin_size, n_hist = brainstate.optim.StepLR(1e-2, step_size=50, gamma=0.9), 0.99, 0.0825 / 100 * u.mV, 0.2 * u.Hz, 20  # brainpy-tower1
        # lr, etrace_decay, scale_factor, bin_size, n_hist = brainstate.optim.StepLR(1e-2, step_size=50, gamma=0.9), 0.99, 0.0825 / 100 * u.mV, 0.4 * u.Hz, 40  # brainpy-tower2
        # lr, etrace_decay, scale_factor, bin_size, n_hist = brainstate.optim.StepLR(1e-2, step_size=50, gamma=0.9), 0.99, 0.0825 / 100 * u.mV, 0.4 * u.Hz, 20  # 智星云V100
    else:
