...
class Trainer:
        ...
        with open(f'{self.filepath}/losses.txt', 'w') as file:
            ...
            for i_epoch in range(train_epoch):
                ...
                for input_neuropil_fr, target_neuropil_fr in self.target.neural_activity.iter_data(
                    ...
                    loss, neuropil_fr, max_g = jax.block_until_ready(res)
                    t1 = time.time()
    ...
    def round2_train(
        ...
        with open(f'{filepath}/losses-round2.txt', 'w') as file:

            # training
            min_loss = np.inf
            for i_epoch in range(train_epoch):
                all_loss = []
                input_neuropil_fr = self.target.neural_activity.spike_rates[:batch_size]
                for i in range(1, self.target.neural_activity.n_time - batch_size):
                    target_neuropil_fr = self.target.neural_activity.spike_rates[i:i + batch_size]

                    t0 = time.time()
                    res = self.train(input_neuropil_fr, target_neuropil_fr)
                    loss, input_neuropil_fr, max_g = jax.block_until_ready(res)
                    t1 = time.time()
                    ...
                    all_loss.append(loss)
                    self.show_res(input_neuropil_fr, target_neuropil_fr, i_epoch, '')
                self.opt.lr.step_epoch()
                ...
                if loss < min_loss:
                    braintools.file.msgpack_save(
                        f'{filepath}/best-checkpoint-round2.msgpack',
                        self.target.states(brainstate.ParamState),
                    )
                    min_loss = loss
...
def example_to_train():
    ...
    trainer.round1_train(100, batch_size=128)


def second_round_train():
    lr = 1e-2
    filepath = 'results/v3/630#2017-10-26_1#100.0Hz#0.99#mse#ETraceParam#ETraceParam#0.000825#20#0.1#2025-03-18-21-02-47'
