...
class NeuralActivity(brainstate.nn.Module):
    ...
    def neuropil_fr_to_embedding(self, neuropil_fr: u.Quantity[u.Hz]):
        ...
        def convert(key, fr):
            ...
            right_bin_indices = self.neuropil_to_bin_indices(fr)
    ...
    def round1_train(
        ...
        with open(f'{filepath}/losses.txt', 'w') as file:
            ...
            for i_epoch in range(train_epoch):
                ...
                all_loss = []
                all_acc = []
                for input_embed, target_neuropil_fr in \
                    self.target.neural_activity.iter_data(
...
def example_to_load():
    ...
    for i in range(n_time):
        ...
        net.simulate(neuropil_fr / u.Hz, indices[:n_sim])
        neuropil_fr = net.simulate(neuropil_fr / u.Hz, indices[n_sim:])
